...
Функция ПолучитьДанныеБУ(НачалоПериода,КонецПериода)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	СУММА(ПартииТоваровНаСкладахБухгалтерскийУчет.Количество) КАК РасходГСМБУКоличество,
		|	СУММА(ПартииТоваровНаСкладахБухгалтерскийУчет.Стоимость) КАК РасходГСМБУСтоимость,
		...
		|		ПО цДокументы2Уровня.ТиповойДокумент = ПартииТоваровНаСкладахБухгалтерскийУчет.Регистратор
		|ГДЕ
		|	ПартииТоваровНаСкладахБухгалтерскийУчет.Номенклатура.ВидНоменклатуры = ЗНАЧЕНИЕ(Справочник.ВидыНоменклатуры.Топливо)
		...
		|	И ПартииТоваровНаСкладахБухгалтерскийУчет.Регистратор ССЫЛКА Документ.ТребованиеНакладная
		|
		|СГРУППИРОВАТЬ ПО
