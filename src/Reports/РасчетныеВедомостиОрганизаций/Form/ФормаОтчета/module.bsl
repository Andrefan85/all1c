...
Процедура ДействияФормыСформировать(Кнопка)
	...
	//rav+ scheglov 26/01/16
	APDEX_ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени("(РАВ)Формирование отчета ""Расчетные ведомости организаций""");	
	//rav-
	
	ОбновитьОтчет();
	
	//rav+ scheglov 26/01/16
	APDEX_ОценкаПроизводительностиКлиентСервер.ЗакончитьЗамерВремени("(РАВ)Формирование отчета ""Расчетные ведомости организаций""");
	//rav-
...
Процедура ДействияФормыРедактироватьНастройкуСтруктуры(Кнопка)
	...
	ФормаНастройки.ОтчетОбъект = ЭтотОбъект;
	...
	Если СохраненнаяНастройка <> Неопределено 
		И ЭтотОбъект.СохраненнаяНастройка.СохранятьАвтоматически Тогда
		СохранитьНастройку();
	КонецЕсли;
...
Процедура ДействияПанелиНачалоВыбораЗначенияТабличногоПоля(Элемент, СтандартнаяОбработка)
	...
	СписокПолей.Добавить("ПодразделениеОрганизации");
	СписокПолей.Добавить("Сотрудник");
	ВладелецСправочника = УправлениеОтчетами.НайтиВладельцаПоФорме(ОтчетОбъект, ЭтаФорма, ЭтаФорма.ТекущийЭлемент, СписокПолей, "Организация");
	Элемент.ВыборПоВладельцу = ВладелецСправочника;
КонецПроцедуры
