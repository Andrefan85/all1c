...
Функция СформироватьОтчет(Результат = Неопределено, ДанныеРасшифровки = Неопределено, ВыводВФормуОтчета = Истина, СтруктураНастроекСМТ = Неопределено) Экспорт
	...
	ТЗЗаправкиДляСвязи = ПолучитьЗаправкиСМТ(ПараметрНачалоПериода.Значение, ПараметрКонецПериода.Значение, ПараметрОрганизация.Значение, СтруктураНастроекСМТ);
	ТЗЗаправкиДляСвязи.Свернуть("Заправщик,ИмяКлюча","ЗаправленоАТЗ,ЗаправленоАЗС");
	
	//Заполним ТЗ заправок, по которым не удалось найти соответствие Имя техники СМТ <--> Имя ключа СМТ
	ТЗЗаправкиБезСоответствия = ТЗЗаправкиДляСвязи.Скопировать();
	ТЗЗаправкиБезСоответствия.Очистить();
	
	Для Каждого Стр Из ТЗЗаправкиДляСвязи Цикл 		
		Если ТЗДанныхПУЛ.Найти(Стр.ИмяКлюча,"НаименованиеТехники") = Неопределено Тогда  
			//Если не нашли по имени ключа технику в исходной таблице, значит связи не будет и выводим её в ТЗ без соответствий
			НовСтр = ТЗЗаправкиБезСоответствия.Добавить();
			ЗаполнитьЗначенияСвойств(НовСтр,Стр);
		КонецЕсли;
	КонецЦикла;
	
	ТЗЗаправкиДляСвязи.Свернуть("ИмяКлюча","ЗаправленоАТЗ,ЗаправленоАЗС");
	ТЗЗаправкиБезСоответствия.Свернуть("Заправщик,ИмяКлюча","ЗаправленоАТЗ,ЗаправленоАЗС");

	ВнешниеНаборыДанных = Новый Структура;
	ВнешниеНаборыДанных.Вставить("ТЗДанныеПутевыелисты",ТЗДанныхПУЛ);
	ВнешниеНаборыДанных.Вставить("ТЗЗаправкиСМТ",ТЗЗаправкиДляСвязи);
	ВнешниеНаборыДанных.Вставить("ТЗЗаправкиСМТБезСоответствия",ТЗЗаправкиБезСоответствия);
...
Функция ПолучитьЗаправкиСМТ(ДатаНачала, ДатаОкончания, Организация, СтруктураНастроекСМТ)
	...
	Иначе		
		...
		СвойствоАТЗ		            = СтруктураНастроекСМТ.СвойствоАТЗ;
		//Токен          				= СтруктураНастроекСМТ.Токен; 
	КонецЕсли;     	
	
	//Определим основные средства и id техники АЗС и АТЗ
	...
	Запрос.Текст = 
		...
		|	""АТЗ""
		|ИЗ
		|	РегистрСведений.ЗначенияСвойствОбъектов КАК СвойствоАТЗ
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗначенияСвойствОбъектов КАК СвойствоИд
		|		ПО СвойствоАТЗ.Объект = СвойствоИд.Объект
		|ГДЕ
		|	СвойствоАТЗ.Свойство = &СвойствоАТЗ
		|	И СвойствоАТЗ.Значение = ИСТИНА
		|	И СвойствоИд.Свойство = &ХранениеИдТехники";
	
	Запрос.УстановитьПараметр("СвойствоАТЗ", СвойствоАТЗ);
	...
	ТЗЗаправок.Колонки.Добавить("Заправщик");
	...
	ТЗЗаправок.Колонки.Добавить("ЗаправленоАТЗ",ОписаниеТиповЧисло);
	...
	ОтборАТЗ = Новый Структура("ПризнакЗаправщика","АТЗ");
	МассивАТЗ =  ТЗЗаправщиков.НайтиСтроки(ОтборАТЗ);
	ЗаправкиАТЗ = ПолучитьЗаправки(МассивАТЗ,ДатаНачала, ДатаОкончания, Организация, СтруктураНастроекСМТ,ТЗЗаправок);  //возвращается ТЗ  	
