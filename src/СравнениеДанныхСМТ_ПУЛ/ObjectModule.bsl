...
Функция ПолучитьТаблицуДанных(ДатаНачала, ДатаОкончания, Организация, СтруктураНастроекСМТ)
	...
	Запрос.Текст = 
		...
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ВТ_Итоговая.Техника,
		|	ВТ_Итоговая.СкладТехника,
		|	ВТ_Итоговая.ТрактористВодитель,
		|	ВТ_Итоговая.РасходГСМПоНорме1С,
		|	ВТ_Итоговая.РасходГСМПоФакту1С,
		|	ВТ_Итоговая.ИдТехники
		|ИЗ
		|	ВТ_Итоговая КАК ВТ_Итоговая
		|ГДЕ
		|	ВТ_Итоговая.Техника <> ЗНАЧЕНИЕ(Справочник.ОсновныеСредства.ПустаяСсылка)";
	...
	ИдОтчетаДляПолученияДанных = "19";
	ИдШаблонаДляПолученияДанных = "2";
	...
	Для каждого Стр Из РезультирующаяТЗ Цикл 
		...
		Если ИдТехники = 0 ИЛИ ИдТехники = "" Тогда
			Продолжить;
		КонецЕсли;
				...
				Если РезультатОтчета.Свойство("stats") Тогда
					...
					Для Каждого ЭлМассива Из СтатистикаОтчета Цикл
						...
						Если ПолеОтчета = "Нач. уровень" Тогда
							Стр.ОстатокНаНачалоСМТ = ПреобразоватьПоказательВЧисло(ЗначениеОтчета);	
						ИначеЕсли ПолеОтчета = "Всего заправлено" Тогда
							Стр.ЗаправакаПоПоказаниямСМТ = ПреобразоватьПоказательВЧисло(ЗначениеОтчета);	
						ИначеЕсли ПолеОтчета = "Потрачено по ДУТ" Тогда
							Стр.РасходГСМПоПоказателямСМТ = ПреобразоватьПоказательВЧисло(ЗначениеОтчета);	
						ИначеЕсли ПолеОтчета = "Конеч. уровень" Тогда
							Стр.ОстатокНаКонецСМТ = ПреобразоватьПоказательВЧисло(ЗначениеОтчета);	
						ИначеЕсли ПолеОтчета = "Всего топлива слито" Тогда
							Стр.СливТопливаПоСМТ = ПреобразоватьПоказательВЧисло(ЗначениеОтчета);	
						КонецЕсли;
...
Функция ПреобразоватьПоказательВЧисло(ЗначениеОтчета)
	
	ВозвращаемоеЧисло = 0;
	
	МаскаЧисел = "0123456789.,";
	ЗначениеЧисло = "";
	Для Ит = 1 По СтрДлина(ЗначениеОтчета) Цикл
		Если Найти(МаскаЧисел,Сред(ЗначениеОтчета,Ит,1)) > 0 Тогда
			ЗначениеЧисло = ЗначениеЧисло + Сред(ЗначениеОтчета,Ит,1);
		КонецЕсли;
	КонецЦикла;
	ВозвращаемоеЧисло = Число(ЗначениеЧисло);  	
